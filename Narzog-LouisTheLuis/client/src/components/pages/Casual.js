...
const Casual = ({ userId, handleLogout }) => {
  ...
  useEffect(() => {
    ...
    get("/api/constellationname").then((constellationname) => {
      get("/api/data").then((constellationdata) => {
        ...
        //Function to randomly select a constellation. 
        ...
        //Creating a mapping of the IAU name to its true name for instance AND and Andromeda, uses an mongodb database  
        ...
        var config = {
          ...
          constellations: {
            ...
            lines: true, // Show constellation lines
            linestyle: { stroke: "#d7c26e", width: 1, opacity: 0.8 },
            bounds: true, // Show constellation boundaries
            boundstyle: { stroke: "#cccc00", width: 0.5, opacity: 0.8, dash: [2, 4] },
          },
          mw: {
        ...
        function submit(e) {
          if (isStart === true) {
            if (counts === 1) {
              counts = 0;
              counting = true;
              //Mapping external canvas coordinate to convert to graphical coordinates in the sphereical map
                  ...
                  if (
                    ...
                    if (area > temp_area) {
                      ...
                      get("/api/using", bodys).then((user) => {
                        post("/api/score", { index: user.score }).then(setScore(score+1)
                        );
                      });
        ...
        function next() {
          ...
          if (isStart === true && greek_constellation_IAU.length != 0 && !occupied) {
            skipcounter = 0;
            occupied = true; 
        ...
        function skip() {
          ...
          if (isStart === true && greek_constellation_IAU.length != 0) {
            skipcounter = 0;
            occupied = true; 
            var lineStyle = {
              stroke: "#f00",
    ...
    <>
      ...
      <div id="background">
        ...
        <div id="gamebox">
          ...
          {!showtitle && (
            <div id="content" className="content-design">
              Welcome to our constellation guessing game. There are a variety of 47 ancient greek
              constellations to learn how to identify in the night sky. The rules are simple: <br />
              1. Press Next to get a new constellation to find. <br />
              2. Try to identify the constellation. <br />
              3. Click twice to create a box around where you think the constellation is <br />
              4. Press Submit if you are pleased with your answer and Cancel if you want to draw a
              new box <br />
              5. Press next for a new one and enjoy!
            </div>
          )}
          {showtitle && <img className="image-design" src={constel}/>}
